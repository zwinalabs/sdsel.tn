		...
		if(count((array) $items)) {
			...
			foreach ($items as $key => $item) {
				...
				$output .= '<div class="sppb-article-info-wrap">';
						...
						if($show_date) {
							$output .= '<span class="sppb-meta-date" itemprop="datePublished">' . Jhtml::_('date', $item->publish_up, 'DATE_FORMAT_LC2') . '</span>';
						}
						
					$output .= '<a href="'. $item->link .'" itemprop="url"><h2>' . $item->title . '</h2></a>';
					...
					if($show_readmore) {
						$output .= '<a class="sppb-readmore" href="'. $item->link .'" itemprop="url">'. $readmore_text .'<i class="fa fa-angle-right"></i></a>';
					}
