...
<div class="j2store-product-images">
	...
	<div class="j2store-thumbnail-image">
		<?php if(JFile::exists(JPATH_SITE.'/'.JPath::clean($this->product->thumb_image))):?>
			...
			<?php endif;?>	
			<img itemprop="image" alt="<?php echo (!empty($this->product->thumb_image_alt)) ? $this->product->thumb_image_alt : $this->product->product_name; ?>" title="<?php echo $this->product->product_name ;?>" class="j2store-img-responsive j2store-product-thumb-image-<?php echo $this->product->j2store_product_id; ?>"  src="<?php echo $image_path.$this->product->thumb_image;?>" />
			
			<?php if($this->params->get('list_image_link_to_product', 1)): ?>
				</a>
			<?php endif;?>
		<?php elseif(!empty($this->product->thumb_image)):?>
			<?php echo J2Store::product()->displayImage($this->product,array('type'=>'Thumb','params' => $this->params,'alt'=>$this->product->thumb_image_alt)); ?>
		<?php endif; ?>
	</div>
	 <?php endif; ?>
	      ...
	      ?>
		   <?php if($main_image &&  JFile::exists(JPATH_SITE.'/'.$main_image)):?>
			...
			<?php endif;?>	
		  	 ...
		  	 <img itemprop="image"
                  alt="<?php echo (!empty($this->product->main_image_alt)) ? $this->product->main_image_alt : $this->product->product_name; ?>" title="<?php echo $this->product->product_name ;?>"
		   ...
		   <?php elseif(!empty($this->product->main_image)):?>
				<?php echo J2Store::product()->displayImage($this->product,array('type'=>'Main','params' => $this->params,'alt'=>$this->product->main_image_alt)); ?>
		   <?php endif; ?>
	</div>
	 <?php endif; ?>
</div>
