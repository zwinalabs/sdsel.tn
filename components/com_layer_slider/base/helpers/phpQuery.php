...
-------------------------------------------------------------------------*/
namespace CreativeSlider\LayerSlider\PHPQuery;
...
class phpQuery {
  ...
  public function __construct($html = null) {
    if ($html) {
      ...
      $this->dom->loadHTML(self::utf8_decode( preg_replace('~>\s+<~', '><', trim($html)) ));
  ...
  public function __get($prop) {
    if ('length' == $prop) return $this->nodes ? $this->nodes->length : 0;
  }
  ...
  public function attr($attr, $value = null) {
    ...
    // setter
    if (is_string($attr)) $attr = array($attr => $value);
    $i = $this->length;
    while ($i--) {
