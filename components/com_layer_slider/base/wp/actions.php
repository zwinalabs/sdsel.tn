...
function ls_erase_plugin_data() {
	...
	if( is_multisite() && isset( ${'_POST'}['networkwide'] ) ) {
		...
		global $lsdb;
		$current = $lsdb->blogid;
		$sites 	 = $lsdb->get_col("SELECT blog_id FROM $lsdb->blogs");
...
function ls_do_erase_plugin_data() {
	...
	$lsdb->query("DROP TABLE {$lsdb->prefix}layerslider;");
	$lsdb->query("DROP TABLE {$lsdb->prefix}layerslider_revisions;");
	...
	foreach( glob($uploadsDir.'layerslider/*/*') as $key => $img ) {
		...
		$attachs = $lsdb->get_col( $lsdb->prepare( "SELECT ID FROM {$lsdb->prefix}posts WHERE guid RLIKE %s;", $imgPath[1] ) );
