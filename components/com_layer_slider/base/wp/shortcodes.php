		...
		if(is_array($slides)) {
			...
			if( ! empty( $GLOBALS['lsPremiumNotice'] ) ) {
				array_unshift($lsContainer, self::generateErrorMarkup(
					...
					sprintf(__('We’ve detected that you’re using premium features in this slider, but you have not yet activated your copy of LayerSlider. Premium features in your sliders will not be available for your visitors without activation. %sClick here to learn more%s. Detected features: %s', 'LayerSlider'), '<a href="http://docs.offlajn.com/creative-slider#activation" target="_blank">', '</a>', implode(', ', $GLOBALS['lsPremiumNotice'])),
