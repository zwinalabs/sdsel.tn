...
function layerslider_create_db_table() {
	...
	// Get DB collate
	if( ! empty($lsdb->charset) ) {
		$charset_collate = "DEFAULT CHARACTER SET $lsdb->charset";
	}

	if( ! empty($lsdb->collate) ) {
		$charset_collate .= " COLLATE $lsdb->collate";
	}

	require_once(ABSPATH . 'wp-admin/includes/upgrade.php');

	// Table for Sliders
	dbDelta("CREATE TABLE {$lsdb->prefix}layerslider (
	...
	dbDelta("CREATE TABLE {$lsdb->prefix}layerslider_revisions (
		  id int(10) NOT NULL AUTO_INCREMENT,
...
function layerslider_verify_db_tables() {
	...
	$layerslider = $lsdb->get_var("SHOW TABLES LIKE '{$lsdb->prefix}layerslider'");
	$layerslider_revisions = $lsdb->get_var("SHOW TABLES LIKE '{$lsdb->prefix}layerslider_revisions'");
	...
	$popup = $lsdb->get_var("SHOW COLUMNS FROM `{$lsdb->prefix}layerslider` LIKE 'flag_popup'");
