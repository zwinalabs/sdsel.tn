	...
	<div class="wrap settings_wrap">
		...
		<div class="title_line">
			...
			<div class="view_title">
				<?php _uge('Slider:', REVSLIDER_TEXTDOMAIN); echo ' '.$slider->getParam("title",""); ?>, 
			
				<?php 
				if($sliderTemplate == "true"){
					_uge("Edit Template Slide",REVSLIDER_TEXTDOMAIN);
				}else{
					_uge("Edit Slide",REVSLIDER_TEXTDOMAIN);
				}
				?> <?php echo $slideOrder?>, <?php _uge("Title:",REVSLIDER_TEXTDOMAIN); ?> <?php echo $slideTitle?>
			
			</div>
			
			<a href="<?php echo GlobalsRevSlider::LINK_HELP_SLIDE?>" class="button-primary float_right revblue mtop_10 mleft_10" target="_blank"><?php _uge("Help",REVSLIDER_TEXTDOMAIN)?></a>			
		...
		<div id="slide_selector" class="slide_selector">
			...
			if($useStaticLayers == 'on'){
				...
				<ul class="list_static_slide_links">
					<li class="revgray nowrap">
						<a href="<?php echo self::getViewUrl(RevSliderAdmin::VIEW_SLIDE,"id=static_$sID"); ?>" class="add_slide"><i class="eg-icon-dribbble"></i><span><?php _uge("Static / Global Layers",REVSLIDER_TEXTDOMAIN)?></span></a>
					</li>
				</ul>
				<?php
			}
			...
			foreach($arrSlideNames as $slidelistID=>$c_slide):
				...
				endforeach;?>
				 ...
				 <li>
					<a id="link_add_slide" href="javascript:void(0);" class="add_slide"><span class="nowrap"><?php _uge("Add Slide",REVSLIDER_TEXTDOMAIN)?></span></a>
				 </li>
		...
		<div class="divide220"></div>
			...
			<span class="float_left ptop_15"> <?php _uge("Choose slide language",REVSLIDER_TEXTDOMAIN)?>: </span> 
			...
			<span class="float_left ptop_15 pleft_20"> <?php _uge("All the language related operations are from",REVSLIDER_TEXTDOMAIN)?> <a href="<?php echo $closeUrl?>"><?php _uge("slides view",REVSLIDER_TEXTDOMAIN)?></a>. </span> 			
		</div>						
		<div class="clear"></div>
		<?php else:?>
		...
		?>
			...
			<div id="slide_params_holder" class="postbox unite-postbox" style="max-width:100% !important;">
				<h3 class="box-closed tp-accordion"><span class="postbox-arrow2">-</span><span><?php _uge("General Slide Settings",REVSLIDER_TEXTDOMAIN) ?></span></h3>
		...
		if(!$slide->isStaticSlide()){
		?>
			<a href="javascript:void(0)" id="button_save_slide" class="revgreen button-primary"><div class="updateicon"></div><?php _uge("Update Slide",REVSLIDER_TEXTDOMAIN)?></a>
		<?php }else{ ?>
			<a href="javascript:void(0)" id="button_save_static_slide" class="revgreen button-primary"><div class="updateicon"></div><?php _uge("Update Static Layers",REVSLIDER_TEXTDOMAIN)?></a>
		<?php } ?>
				
		<span id="loader_update" class="loader_round" style="display:none;"><?php _uge("updating",REVSLIDER_TEXTDOMAIN)?>...</span>
		<span id="update_slide_success" class="success_message" class="display:none;"></span>
		
		<?php if($permission_setting): // permission ?>
		<a href="<?php echo self::getViewUrl(RevSliderAdmin::VIEW_SLIDER,"id=$sliderID");?>" class="button-primary revblue"><?php _uge("To Slider Settings",REVSLIDER_TEXTDOMAIN)?></a>
		<?php endif; // end permission ?>
		
		<a id="button_close_slide" href="<?php echo $closeUrl?>" class="button-primary revyellow"><div class="closeicon"></div><?php _uge("To Slide List",REVSLIDER_TEXTDOMAIN)?></a>
		...
		if(!$slide->isStaticSlide()){
		?>
		<a href="javascript:void(0)" id="button_delete_slide" class="button-primary revred" original-title=""><i class="revicon-trash"></i><?php _uge("Delete Slide",REVSLIDER_TEXTDOMAIN)?></a>
		<?php } ?>
