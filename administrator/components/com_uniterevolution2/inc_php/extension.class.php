...
class RevSliderExtension {
	...
	public function add_eg_additional_meta_field($values){
		...
		?>
		<p>
			<strong style="font-size:14px"><?php _uge('Choose Revolution Slider', REVSLIDER_TEXTDOMAIN); ?></strong>
		</p>
		<p>
			<select name="eg_sources_revslider" id="eg_sources_revslider">
				<option value=""<?php selected($selected_slider[0], ''); ?>><?php _uge('--- Choose Slider ---', REVSLIDER_TEXTDOMAIN); ?></option>
				<?php
				if(!empty($shortcodes)){
