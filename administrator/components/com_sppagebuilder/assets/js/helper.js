...
(function ($) {
	...
	$.fn.columnSortable = function() {
		...
		$( this ).sortable({
			...
			stop: function( event, ui ){
				if (ui.item.hasClass('section-inner')) {
					if ($(ui.item).parent()[0].hasClass('inner-col')) {
						$( this ).sortable( 'cancel' );
					}
				}
			},
			over: function( event, ui ) {
				if ($(ui.placeholder).parent()[0].hasClass('inner-col')) {
					if (ui.item.hasClass('section-inner')) {
	...
	$.fn.cloneRepeatable = function()
	{
		...
		$('.repeatable-items').find('.sppb-editor').each(function(){
			var $id = $(this).attr('id');
			tinymce.execCommand('mceRemoveEditor', true, $id);
		});
		$('.repeatable-items').find('select').chosen('destroy'); //Destroy Chosen
		...
		$('.repeatable-items').randomIds();
		$('.repeatable-items').find('select').chosen({allow_single_deselect:true}); //Chosen
		$('.repeatable-items').find('.sppb-editor').each(function(){
			var $id = $(this).attr('id');
